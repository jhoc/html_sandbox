...
export function resetKeyboardColors() {
    ...
    for (let i = 0; i < elemKey.children.length; i++) {
        if (elemKey.children[i] == undefined)
            continue;
        elem = elemKey.children[i];
...
function htmlForKeyboardWithOctaves(numberOfOctaves, startOctave, showLabels, withShiftButtons, withNoteSelection, withClefSelection) {
    if (typeof (numberOfOctaves) === 'undefined')
        numberOfOctaves = 3;
    if (typeof (startOctave) === 'undefined')
        startOctave = octaves.C4;
    if (typeof (showLabels) === 'undefined')
        showLabels = true;
    if (typeof (withShiftButtons) === 'undefined')
        withShiftButtons = true;
    if (typeof (withNoteSelection) === 'undefined')
        withNoteSelection = true;
    if (typeof (withClefSelection) === 'undefined')
        withClefSelection = true;
...
function limitToRange(_number, _min, _max) {
